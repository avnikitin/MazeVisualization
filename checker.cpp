...
int main(int argc, char *argv[]) {
	...
	double xsz = szX, ysz = szY;
	std::vector <Segment> borders = { Segment({0,0}, {xsz, 0}), Segment({xsz, 0}, {xsz, ysz}),
									  Segment({xsz, ysz}, {0, ysz}), Segment({0, ysz}, {0, 0}) };
	for (int i = 0; i < N; i++) {
		for (const Segment& border : borders) {
			if (doSegmentAndCircleIntersect(border, vertices[i].first)) {
				std::wcout << L"vertice " << vertices[i].second << L" intersects with border";
				exit(1);
			}
		}
	}
	for (int i = 0; i < wallsN; i++) {
		if (less(walls[i].first.get_ld().get_x(), 0) || greater(walls[i].first.get_ru().get_x(), szX)) {
			std::cout << ord(i + 1) << " wall intersects with border";
			exit(1);
		}
		if (less(walls[i].first.get_ld().get_y(), 0) || greater(walls[i].first.get_ru().get_y(), szY)) {
			std::cout << ord(i + 1) << " wall intersects with border";
			exit(1);
		}
	}
	for (int i = 0; i < edges.size(); i++) {
		for (const Segment& border : borders) {
			if (doSegmentsIntersect(edges[i].first, border)) {
				if (edges[i].first.get_r().get_x() == 12345) {
					continue;
				}
				std::cout << L"path between vertices " << edges[i].second[0] << L" and " \
					<< edges[i].second[1] << L" intersects with border";
				exit(1);
			}
		}
	}
